...
ggplot(measures.bf,aes(x=length,y=normtime,colour=factor(languagerepresentation)))+ geom_line() + geom_point(aes(shape=datastructure)) +  ggtitle("Bitflip")+scale_y_log10()+scale_x_log10()+scale_color_manual(name='Language',values=getPalette(colourCount))+ theme(legend.title = element_text(colour="blue", size=9, face="bold"))+ theme(legend.text = element_text(colour="blue", size=7))+theme_tufte(ticks=FALSE)
ggsave("bitflip-norm.pdf", width = 297, height = 210, units = "mm")
...
ggsave("xover-norm.pdf", width = 297, height = 210, units = "mm")
...
ggsave("maxones-norm.pdf", width = 297, height = 210, units = "mm")
